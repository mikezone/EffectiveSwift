...
public extension NSMutableAttributedString {
    ...
    func setAttribute(_ attributeName: String, value: Any?, range: NSRange? = nil) {
        if let value = value {
            self.addAttribute(NSAttributedStringKey(rawValue: attributeName), value: value, range: range ?? self.rangeOfAll)
        } else {
            self.removeAttribute(NSAttributedStringKey(rawValue: attributeName), range: range ?? self.rangeOfAll)
        }
    }
    
    func setFont(_ font: UIFont, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.font.rawValue, value: font, range: range)
    }
    
    func setKern(_ kern: CGFloat, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.kern.rawValue, value: kern, range: range)
    }
    
    func setForegroundColor(_ foregroundColor: UIColor, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.foregroundColor.rawValue, value: foregroundColor, range: range)
    }
    
    func setBackgroundColor(_ backgroundColor: UIColor, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.backgroundColor.rawValue, value: backgroundColor, range: range)
    }
    
    func setStrokeWidth(_ strokeWidth: CGFloat, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.strokeWidth.rawValue, value: strokeWidth, range: range)
    }
    
    func setStrokeColor(_ strokeColor: UIColor, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.strokeColor.rawValue, value: strokeColor, range: range)
    }
    
    func setShadow(_ shadow: NSShadow, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.shadow.rawValue, value: shadow, range: range)
    }
    ...
    func setStrikethroughStyle(_ underlineStyle: NSUnderlineStyle, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.strikethroughStyle.rawValue, value: underlineStyle.rawValue, range: range)
    }
    
    /// NSUnderlineStyleAttributeName(下划线)
    /// 该属性所对应的值是一个 NSNumber 对象(整数)。该值指定是否在文字上加上下划线，该值参考“Underline Style Attributes”。默认值是NSUnderlineStyleNone
    func setUnderlineStyle(_ underlineStyle: Int, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.underlineStyle.rawValue, value: underlineStyle, range: range)
    }
    ...
    func setUnderlineColor(_ underlineColor: UIColor, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.underlineColor.rawValue, value: underlineColor, range: range)
    }
    ...
    func setLigature(_ ligature: Int, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.ligature.rawValue, value: ligature, range: range)
    }
    
    func setTextEffect(_ textEffect: String, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.textEffect.rawValue, value: textEffect, range: range)
    }
    
    /// 字体倾斜
    func setObliqueness(_ obliqueness: CGFloat, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.obliqueness.rawValue, value: obliqueness, range: range)
    }
    
    /// 文本扁平化
    func setExpansion(_ expansion: CGFloat, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.expansion.rawValue, value: expansion, range: range)
    }
    
    func setBaselineOffset(_ baselineOffset: CGFloat, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.baselineOffset.rawValue, value: baselineOffset, range: range)
    }
    
    /// 0 means horizontal text.  1 indicates vertical text.
    func setVerticalGlyphForm(_ verticalGlyphForm: Bool, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.verticalGlyphForm.rawValue, value: verticalGlyphForm ? 1 : 0, range: range)
    }
    ...
    func setWritingDirection(_ writingDirection: Int, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.writingDirection.rawValue, value: writingDirection, range: range)
    }
    
    func setParagraphStyle(_ paragraphStyle: NSParagraphStyle, _ range: NSRange? = nil) {
        self.setAttribute(NSAttributedStringKey.paragraphStyle.rawValue, value: paragraphStyle, range: range)
    }
